...
//
	...
	for (y=0;y<mapheight;y++)
		for (x=0;x<mapwidth;x++)
		{
			...
			// area floor
				tilemap[x][y] = 0;
				//this is 0x01 with the map discovered.
				automap[x][y] = 0x00;
...
//
	...
	for (y=0;y<mapheight;y++)
		...
		{
			tile = *map++;
			//inject typemap data while you're at it
			//typemap[x][y]=tile;
			
			//put exclusions here
			
			//colorize the typemap!!
		
		//LEVEL CODES
			if(tile >= 0 && tile <= 63){       //walls
				typemap[x][y] = 0x7d;
			}
			else if(tile >= 90 && tile <= 91){ //regular doors
				typemap[x][y] = 0x03;
			}
			else if(tile >= 92 && tile <= 93){ //gold key doors
				typemap[x][y] = 0xa3;
			}
			else if(tile >= 94 && tile <= 95){ //silver key doors
				typemap[x][y] = 0x16;
			}
			else if(tile >= 100 && tile <= 101){ //elevators
				typemap[x][y] = 0x05;
			}
			else if(tile >= 106 && tile <= 143){ //floors
				typemap[x][y] = 0x1d;
			}
