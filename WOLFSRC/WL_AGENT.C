...
= ControlMovement
=
= Takes controlx,controly, and buttonstate[bt_aim]
=
...
void ControlMovement (objtype *ob)
{
	...
	//aiming
	if (buttonstate[bt_aim])
	{
		if(facedistmod<4)
		{
			facedistmod=facedistmod+0.1;
		}
	}
	else
	{
...
//
	...
	//
		if (controlx > 0)
		{
			...
			if (angle < 0)
				angle += ANGLES;
			Thrust (angle,(controlx*MOVESCALE)*0.5);	// move to left
		}
		else if (controlx < 0)
		{
			angle = ob->angle + ANGLES/4;
			if (angle >= ANGLES)
				angle -= ANGLES;
			Thrust (angle,(-controlx*MOVESCALE)*0.5);	// move to right
		}
	//}
//	else
//	{
...
//automap
	aplayerx = player->tilex;
	aplayery = player->tiley;
}
...
//
	...
	while (gamestate.attackcount <= 0)
	{
		...
		switch (cur->attack)//                                                                    Is the player attacking?
		{
		case -1://                                                                                    Player isn't attacking
			ob->state = &s_player;
			if (!gamestate.ammo)//Out of ammo,
			{
				gamestate.weapon = wp_knife;//switch to knife
				DrawWeapon ();//draw the knife
			}
			else
			{
				if (gamestate.weapon != gamestate.chosenweapon)//If I've chosen a different weapon,
				{
					gamestate.weapon = gamestate.chosenweapon;//switch to the new weapon
					DrawWeapon ();//draw the new weapon
				}
			};
